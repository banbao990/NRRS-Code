...
__global__ void top_k_gpu_kernel3_1(DATATYPE *input, int length, int k, DATATYPE *output) {
	...
	for (index = 0, i = blockIdx.x * localThreadNum * k + localThreadId; index < k;
		 index++, i += localThreadNum) {
		myPoint[index] = NEG_INF;
		replace_smaller(myPoint, index + 1, input[i]);
	}
	// replace the data if bigger
...
__global__ void top_k_gpu_kernel3_2_orig(DATATYPE *input, int num, int stride, int k,
										 DATATYPE *output) {
	int threadId	   = blockIdx.x * blockDim.x + threadIdx.x;
	int threadNum	   = gridDim.x * blockDim.x;
	int localThreadId  = threadIdx.x;
...
__global__ void top_k_gpu_kernel4_2(DATATYPE *input, int num, int skip_stride, int k,
									DATATYPE *output) {
	extern __shared__ DATATYPE shared_buffer[];
	DATATYPE *myPoint  = shared_buffer + threadIdx.x;
	int threadId	   = blockIdx.x * blockDim.x + threadIdx.x;
...
void top_k_gpu(DATATYPE *input, int length, int k, DATATYPE *output, CUstream stream) {
	...
	top_k_gpu_kernel1<<<blocks, threads, 0, stream>>>(input, length, k, output);
	...
	top_k_gpu_kernel2<<<blocks, threads, shared_mem_usage, stream>>>(input, length, k, output);
	cudaError_t err = cudaGetLastError();
	HANDLE_CUDA_ERROR(err);
#endif
	...
	int blocks =
		(blocks_opt < length / (threads * 2 * k)) ? blocks_opt : (length / (threads * 2 * k));
	int shared_mem_usage = sizeof(DATATYPE) * k * threads;
	...
	if (shared_mem_usage < GPU_SHARED_MEM_THRESHOLD)
		top_k_gpu_kernel3_1<<<blocks, threads, shared_mem_usage, stream>>>(input, length, k,
																		   output);
	else
		top_k_gpu_kernel3_1_orig<<<blocks, threads, 0, stream>>>(input, length, k, output);
	threads			 = (thread_opt < blocks / 2) ? thread_opt : (blocks / 2);
	shared_mem_usage = sizeof(DATATYPE) * k * threads;
	// printf("shared mem usage: (%d %d) %d(%d)\n", 1, threads, shared_mem_usage,
	// GPU_SHARED_MEM_THRESHOLD);
	if (shared_mem_usage < GPU_SHARED_MEM_THRESHOLD)
		top_k_gpu_kernel3_2<<<1, threads, shared_mem_usage, stream>>>(input, blocks, stride, k,
																	  output);
	else
		top_k_gpu_kernel3_2_orig<<<1, threads, 0, stream>>>(input, blocks, stride, k, output);
	cudaError_t err = cudaGetLastError();
	HANDLE_CUDA_ERROR(err);
#endif
#if 0
    ...
    int shared_mem_usage = sizeof(DATATYPE)*k*threads;
    if(shared_mem_usage < GPU_SHARED_MEM_THRESHOLD)
        top_k_gpu_kernel4_1<<<blocks, threads, shared_mem_usage, stream>>>(input, length, k, output);
    else
        printf("%d %d %d\n", blocks, threads, shared_mem_usage);
    threads = (GPU_THREADS < blocks / 2) ? GPU_THREADS : (blocks / 2);
    shared_mem_usage = sizeof(DATATYPE)*k*threads;
    if(shared_mem_usage < GPU_SHARED_MEM_THRESHOLD)
        top_k_gpu_kernel4_2<<<1, threads, shared_mem_usage, stream>>>(input, blocks, stride, k, output); 
    else
        printf("%d %d %d\n", blocks, threads, shared_mem_usage);
    cudaError_t err = cudaGetLastError();
    HANDLE_CUDA_ERROR(err);
