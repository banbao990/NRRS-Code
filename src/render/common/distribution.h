...
public:
	...
	KRR_CALLABLE float eval(const Vector3f &wi) const { return lambda * pdf(wi); }
	...
	KRR_CALLABLE void product(const VMFKernel &other) {
		...
		if (new_kappa >= 1e-3) // for numerical stability.
			new_mu /= new_kappa;
		else { // only update kappa & mu when numerical stability is promised.
			new_kappa = 0;
			new_mu	  = mu_a;
		}
		Vector2f new_mu_sph = utils::cartesianToSpherical(new_mu);
		// this is the numerical unstable version (since sinh grows exponentially)
		// s = M_INV_4PI * kappa * other.kappa / new_kappa * sinh(new_kappa) / sinh(kappa) /
		// sinh(other.kappa);
	...
	// it is suggested that the kappa should be no less than 1e-3 for numerical stability.
	static constexpr float minValidKappa = 1e-3f;

	float lambda; // [0, +inf), clamped to exp(-+10)
	float kappa;  // [0, +inf), clamped to exp(-+10);
	float theta;  // [0, pi], maybe it not needed to clamp?
	float phi;	  // [0, 2pi]
};

template <int N> class MixedSphericalGaussianDistribution {
public:
	enum {
		N_COMP = N
	};
	...
	KRR_CALLABLE MixedSphericalGaussianDistribution(T *data) {
		...
		for (int i = 0; i < N; i++) {
			...
			mSG[i].theta =
				(float) M_PI * network_to_params((float) data[idx + 2], ACTIVATION_THETA);
	...
	// Evaluate function of the i-th vMF component only.
	...
	KRR_CALLABLE float gradients_probability(const Vector3f wi, T *output) const {
		...
		float probability = 0; // calculate the probability by the way...
		for (int sg = 0; sg < N; sg++) {
			...
			// So check the implementation of exponential activation, which uses a clamp to force
			// x>0!
			float lambda = mSG[sg].lambda, kappa = mSG[sg].kappa, theta = mSG[sg].theta,
				  phi = mSG[sg].phi;
			// Check the numerical stable version of the derivatives!
			float vmf = pdf(sg, wi);
			probability += weight[sg] * vmf;

			/* [1] dLoss_dlambda, which is dependent on all vmf components */
			float dF_dlambda = vmf * (totalWeight - lambda) / pow2(totalWeight);
			for (int k = 0; k < N; k++) {
				if (k != sg) dF_dlambda -= weight[k] / totalWeight * mSG[k].pdf(wi);
			}
			/* [2] dLoss_dkappa */
			float dF_dkappa =
				weight[sg] * vmf *
				(x * sin(theta) * cos(phi) + y * sin(theta) * sin(phi) + z * cos(theta) +
				 1 / kappa - (1 + expf(-2 * kappa)) / (1 - expf(-2 * kappa)));
			/* [3] dLoss_dtheta */
			float dF_dtheta = weight[sg] * vmf * kappa *
							  (cos(theta) * (x * cos(phi) + y * sin(phi)) - z * sin(theta));
			/* [4] dLoss_dphi */
	...
	/* Apply the cosine lobe (vmf approximation) for each vmf component. */
...
template <int N> class MixedSGWithSelection : public MixedSphericalGaussianDistribution<N> {
public:
	MixedSGWithSelection() = default;

	template <typename T>
	// maps network's raw output to valid parameters
	KRR_CALLABLE MixedSGWithSelection(T *data) : MixedSphericalGaussianDistribution<N>(data) {
		selectionProb = extractSelectionProbability(data);
	}

	KRR_CALLABLE float pdf(const Vector3f &wi,
						   float bsdfPdf /* To be mixed with the defensive strategy */) {
		return selectionProb * MixedSphericalGaussianDistribution<N>::pdf(wi) +
			   (1 - selectionProb) * bsdfPdf;
	}

	template <typename T>
	KRR_CALLABLE float gradients_probability(const Vector3f wi, const float bsdfPdf,
											 T *output) const {
		float probability =
			MixedSphericalGaussianDistribution<N>::gradients_probability(wi, output);
		for (int i = 0; i < N * N_DIM_VMF; i++) output[i] = (float) output[i] * selectionProb;
	...
	template <typename T> KRR_CALLABLE static float extractSelectionProbability(T *data) {
