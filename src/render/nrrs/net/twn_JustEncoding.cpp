...
void TwoHeadedNetwork_JustEncoding::reset() {
	json encoding_opts	= m_config.value("encoding", json::object());
	json optimizer_opts = m_config.value("optimizer", json::object());

	json twoNet_config = m_config.value("two_head_net_just_encoding", json::object());
	...
	json net12_opts = twoNet_config.value("net", json::object());
	m_dims_transfer = n_neurons;

	std::cout << "[TwoHeadedNetwork_JustEncoding] Set Internal Dimension: " << m_dims_transfer
			  << std::endl;

	// encoding: no loss
	m_encoding.reset(create_encoding<precision_t>(m_n_input_dims, encoding_opts));
	m_encoding->set_alignment(minimum_alignment(net12_opts));

	m_optimizer_encoding.reset(create_optimizer<precision_t>(optimizer_opts));
	m_trainer_base = std::make_shared<Trainer_Float>(m_encoding, m_optimizer_encoding, nullptr);

	// network
	net12_opts["n_input_dims"]	= m_encoding->padded_output_width();
...
TwoHeadedNetwork_JustEncoding::TwoHeadedNetwork_JustEncoding(const json &config,
...
void TwoHeadedNetwork_JustEncoding::set_max_training_batch_size(const uint32_t batch_size) {
...
void TwoHeadedNetwork_JustEncoding::set_max_inference_batch_size(const uint32_t batch_size) {
...
void TwoHeadedNetwork_JustEncoding::inference(cudaStream_t stream, const GPUMatrix_Float &input,
	...
	m_encoding->inference_mixed_precision(stream, input, transfer_l);
...
float TwoHeadedNetwork_JustEncoding::training_step(cudaStream_t stream,
												   const GPUMatrix_Float &input,
												   const GPUMatrix_Float &target,
												   const bool get_loss, precision_t *&output1,
												   precision_t *&output2) {
...
void TwoHeadedNetwork_JustEncoding::updateLossHyperparams(const json &config) {
	m_loss_heads[0]->update_hyperparams(config);
	m_loss_heads[1]->update_hyperparams(config);
}

void TwoHeadedNetwork_JustEncoding::print_info() const {
	int net_params = 0;
	net_params += m_encoding->n_params();
	net_params += m_net_heads[0]->n_params();
	net_params += m_net_heads[1]->n_params();

	int encoding_params = m_encoding->n_params();
	...
	optimizer_params += m_optimizer_encoding->n_weights();
