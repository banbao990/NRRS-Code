...
__global__ void adn_generate_inference_data(const size_t nElements, ADNInferenceQueue *inferQueue,
	...
	const ADNInferenceWorkItem &item = inferQueue->operator[](i).operator ADNInferenceWorkItem();

	Vector3f pos				  = normalizeSpatialCoord(item.mPos, sceneAABB);
	*(Vector3f *) &data[data_idx] = pos;
	*(Vector2f *) &data[data_idx + NRC_DIM_SPATIAL_INPUT] = item.mDir;
